// ═══════════════════════════════════════════════
// AllNet — send-spray-notification Edge Function
...
// ── Web Push crypto helpers (shared with send-court-alert) ──
...
Deno.serve(async (req) => {
  try {
    ...
    if (!record?.match_id || !record?.user_id || !record?.spray_item_id) {
      return new Response(JSON.stringify({ error: 'Invalid record' }), { status: 400 })
    }

    const { match_id, user_id: sprayerUserId, spray_item_id } = record

    console.log(`Spray notification: ${sprayerUserId} sprayed match ${match_id}`)

    const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)

    // 1. Get sprayer's name
    const { data: sprayer } = await supabase
      .from('profiles')
      .select('name, first_name, last_name')
      .eq('id', sprayerUserId)
      .single()

    const sprayerName = sprayer?.first_name && sprayer?.last_name
      ? `${sprayer.first_name} ${sprayer.last_name}`
      : (sprayer?.name || 'Someone')

    // 2. Get spray item name
    const { data: sprayItem } = await supabase
      .from('store_items')
      .select('name')
      .eq('id', spray_item_id)
      .single()
    ...
    // 3. Get match details (court name + participants)
    const { data: match } = await supabase
      .from('match_feed')
      .select('court_name, team_a, team_b')
      .eq('id', match_id)
      .single()

    if (!match) {
      console.log('Match not found')
      return new Response(JSON.stringify({ skipped: true, reason: 'match not found' }))
    }

    // 4. Extract all participant user_ids from both teams
    ...
    const participantIds = [...new Set(
      allPlayers
        .filter(p => p.user_id && p.user_id !== sprayerUserId)
        .map(p => p.user_id)
    )]

    if (participantIds.length === 0) {
      console.log('No participants to notify (sprayer is only real user)')
    ...
    // 5. Check push_spray_alerts preference for each participant
    const { data: profiles } = await supabase
      .from('profiles')
      .select('id, push_spray_alerts')
      .in('id', participantIds)

    const eligibleIds = (profiles || [])
      .filter(p => p.push_spray_alerts !== false) // default true
      .map(p => p.id)

    if (eligibleIds.length === 0) {
      console.log('All participants have spray alerts disabled')
      return new Response(JSON.stringify({ skipped: true, reason: 'all opted out' }))
    }

    // 6. Insert in-app notification records for all eligible users
    const notificationTitle = `${sprayerName} sprayed your post`
    const notificationBody = `Used ${sprayName} on your game at ${match.court_name}`
    ...
    if (notifError) {
      ...
      console.log(`Inserted ${notifRows.length} in-app notifications`)
    }

    // 7. Get push subscriptions for eligible users
    ...
    if (!subscriptions || subscriptions.length === 0) {
      console.log('No push subscriptions for eligible participants')
    ...
    // 8. Build push payload and send
    const pushPayload = {
      title: notificationTitle,
      body: notificationBody,
      matchId: match_id,
      type: 'post_sprayed',
      url: '/allnet-activity.html'
    }
    ...
    console.error('send-spray-notification error:', err)
